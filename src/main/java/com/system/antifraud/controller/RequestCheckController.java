...
import com.system.antifraud.model.request.CheckCreate;
import com.system.antifraud.model.request.CheckRequest;
import com.system.antifraud.model.request.TransactionList;
...
public class RequestCheckController {
    ...
    public String resultCheck(@RequestBody TransactionList Tl, Model model){
        if(Tl.getTrid() == null || Tl.getDadd() == null || Tl.getRegion() == null || Tl.getClRecip() == null || Tl.getClSender() == null){
        ...
        transaction.setDadd(Tl.getDadd());
        ...
        if(!clientRepository.existsById(Tl.getClSender().getClid())){
            Client clientSend = new Client(Tl.getClSender().getClid(),Tl.getClSender().getSurname(),Tl.getClSender().getName(),Tl.getClSender().getPatronymic());
        ...
        if(!clientRepository.existsById(Tl.getClRecip().getClid())) {
            Client clientRecip = new Client(Tl.getClRecip().getClid(), Tl.getClRecip().getSurname(), Tl.getClRecip().getName(), Tl.getClRecip().getPatronymic());
        ...
        CheckFraud checkFraud = new CheckFraud(chekid, format.format(new Date()), Tl.getTrid(), null, null);
